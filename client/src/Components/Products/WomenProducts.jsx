...
export default function WomenProductsPage() {
  ...
  const [availableStores, setAvailableStores] = useState([]);     // Only stores with women's products
  const [availableCategories, setAvailableCategories] = useState([]); // Only categories with women's products

  const [currentPage, setCurrentPage] = useState(1);
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const productsPerPage = 9;

  // Fetch women's & unisex products from featured
  useEffect(() => {
    const fetchWomenProducts = async () => {
      try {
        ...
        // Filter for women & unisex products only
        ...
        // Extract available stores & categories from women's products
        const uniqueStores = [...new Set(womenProducts.map(p => p.store))].filter(Boolean);
        const uniqueCategories = [...new Set(womenProducts.map(p => p.category))];
        ...
        if (womenProducts.length > 0) {
          const highest = Math.max(...womenProducts.map(p => p.price));
    ...
    <>
      <div className="min-h-screen pt-20 pb-10" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <div className="max-w-7xl mx-auto px-6 pt-8">
          <h1 className="text-4xl md:text-5xl font-light text-gray-900">{t.title || "Women's Collection"}</h1>
