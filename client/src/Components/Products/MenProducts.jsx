...
export default function MenProductsPage() {
  ...
  // Fetch only men's & unisex featured products
  useEffect(() => {
    const fetchMenProducts = async () => {
      try {
        ...
        // Filter for men & unisex only + showOnProductsPage
        ...
        const uniqueCategories = [...new Set(menProducts.map((p) => p.category).filter(Boolean))];
  ...
  // Filtered products (only category filter now)
